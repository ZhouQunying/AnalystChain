...
if __name__ == '__main__':
    gdp_df = pd.read_csv('gdp_yearly_processed.csv')
    cpi_df = pd.read_csv('cpi_processed.csv')
    pmi_df = pd.read_csv('pmi_processed.csv')

    economic_stage, recommendations = analyze_economic_cycle(gdp_df, cpi_df, pmi_df)

    with open('economic_analysis_report.txt', 'w', encoding='utf-8') as f:
