...
|---|------|------|-----|-----|----------|------|
| 1 | 环境准备 | 验证依赖和文件 | 确保环境可用 | 验证analyst_chain环境已激活+data/raw/knowledge/macroeconomic/有知识文件 | 依赖完整+文件就绪 | ✅ |
...
def process_all_knowledge():
    # 1. 组织文件（模块1）
    organizer = KnowledgeOrganizer(Path("data/raw/knowledge/macroeconomic/"))
